...
class SubsampleLayer(layers.Layer):
    def __init__(self, incoming, window=(None, None, None), **kwargs):
        super(layers.Layer, self).__init__(incoming, **kwargs)
        self.window = window

    def get_output_shape_for(self, input_shape):
        output_shape = list(input_shape)
        output_shape[2] = input_shape[2] / self.window[2]
        return tuple(output_shape)

    def get_output_for(self, input, **kwargs):
        start, stop, step = self.window
        return input[:, start:stop, (step - 1)::(step)]
